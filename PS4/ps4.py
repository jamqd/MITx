...
def r_squared(y, estimated):
    ...
    for i in range(len(y)):
        topSum += (y[i] - estimated[i]) ** 2
    botSum = 0
    for o in y:
        botSum += (o - mean) ** 2
    return 1 - topSum/botSum
    ...
    for n in models:
        ...
        for i in x:
            ...
            for k in range(len(n)):
                estY += n[k] * (i ** (len(n) - 1 - k))
            est.append(estY)
        rsq = r_squared(y, est)
        print(est)
        modLabel = 'Model ' + '$(R^2 = $' + str(round(rsq, 3)) + '): y = '
        for z in range(len(n)):
            if n[z] >= 0 and z != 0:
                modLabel += ' +'
            if z == len(n) - 1:
                modLabel += '$ ' + str(round(n[z], 3)) + '$'
            elif z == len(n) - 2:
                modLabel += '$ ' + str(round(n[z], 3)) + 'x' + '$'
            else:
                modLabel += '$ ' + str(round(n[z], 3)) + 'x^' + str(len(n) - 1 - z) + '$'
        pylab.plot(x,y, label = 'Temperature Data', linestyle = '', marker = 'o')
        pylab.plot(x,est, label = modLabel)
        pylab.title('Degree ' + str(len(n) - 1) + ' Polynomial Model for Temperature (' +
                    str(x[0]) + '-' + str(x[len(x) - 1]) + ')')
        pylab.xlabel('Year')
