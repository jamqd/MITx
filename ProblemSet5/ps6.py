...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        shiftStringL = lettersL[shift:26] + lettersL[0: shift]
        count = 0
        for n in lettersL:
            shiftDict[n] = shiftStringL[count]
            count += 1
        lettersU = string.ascii_uppercase
        shiftStringU = lettersU[shift:26] + lettersU[0: shift]
        count = 0
        for n in lettersU:
            shiftDict[n] = shiftStringU[count]
            count += 1
        return shiftDict
...
class PlaintextMessage(Message):
    ...
    def change_shift(self, shift):
        ...
        self.shift = shift
        self.encrypting_dict  = Message.build_shift_dict(self, self.shift)
        Message.apply_shift(self, self.shift)
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        bestShift = 0
        for n in range (26):
            words = self.apply_shift(26-n).split(' ')
            countWords = 0
            for i in words:
                if i in self.get_valid_words():
                    countWords += 1
            if countWords > maxValidWords:
                maxValidWords = countWords
                bestShift = 26 - n
        if bestShift == 26:
            bestShift = 0
